...
One valuable feature of EukDetect is its ability to reliably report a pair of similar species. The authors demonstrate this through simulating a mixture of two *Entamoeba* species - parasitic *E. histolytica* and commensal *E. dispar* - at varying coverage, and showing that starting from 0.05 - 0.1 coverage, the tool reliably reports both species in a simulated mixture. We wanted to make sure CORRAL retains this ability, and investigate if any pairs of species are particularly hard to report as a mixture.

Our starting point is the database of alignments of simulated reads that we prepared for Figure 1. We ignore all reads that map to their source taxa, extract 7173 pairs of taxa that cross-map, and for each taxon of a pair, compute rates at which the taxon either emits or accepts reads for either the other member or elsewhere. We then apply principal component analysis to this eight-dimensional feature space, and take the first two components (which explain 62% and 18.9% of the variance). This gives us a two-dimensional 'difficulty space' in which pairs that are similarly problematic to distinguish are near each other as points. We select 114 points through geospatial subsampling, simulate equal-coverage mixes at 0.1 coverage, and run both EukDetect and CORRAL.

The results are absolutely epic.
