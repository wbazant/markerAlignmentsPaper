...
We return to the alignments of simulated reads that we created for Figure 1, retain those that did not map to those taxon of origin, and summarise them by source and mapped taxon, giving us mismatch frequencies for 7173 pairs of taxa. We then pick representative 114 pairs, simulate equal-coverage mixes at levels of 0.01, 0.05, and 0.10, and run both EukDetect and CORRAL. The results can be seen on Figure 4b - in each subchart, a dot is a pair of taxa, dots that are near each other on the chart are similar in mismatch frequency, and rate of mismatches increases from left to right. The colors are yellow for one or both species not reported (false negative), blue for a true positve (both species reported) and no false positives (no other species reported), and purple for a true positive with additional false positives.

Neither method has good sensitivity at detecting pairs at 0.01 coverage, although CORRAL can mostly handle the easier cases. For coverages of 0.05 and 0.10, CORRAL reports sources of reads in all cases, but it also reports additional false positives. Meanwhile, EukDetect mostly reports the pairs correctly at 0.05, but produces false negatives for the hard cases, and at coverage of 0.10 it can manage some more of the hard cases, but starts to produce false negatives elsewhere.


Method:
To analyse pairs of taxa which can get confused with each other at read mapping level, we use the same database of alignments of simulated reads that we prepared for Figure 1. We take all pairs of taxa such that a read sampled from a source taxon is aligned to a different taxon, and for each member of a pair, compute rates at which the taxon either emits or accepts reads for either the other member or elsewhere. We then apply principal component analysis to this eight-dimensional feature space, and take the first two components. The first principal component explains 62% of the variance, its coefficients are positive for all features, and symmetric between the pair members, so we interpret it as general difficulty of telling the two species apart. The second component explains 18.9% of the variance and its coefficients are also symmetric between the members, and we do not assign it a meaning. For the purposes of plotting, we interpret the two components together as a two-dimensional 'difficulty space' in which pairs that are similarly problematic are near each other as points. We seed a greedy geospatial subsampling algorithm with *E. histolytica* and *E. dispar* used in the original EukDetect publication, set it to skip the taxa that aren't at species level, choose the distance parameter of 0.05, and obtain 113 other points.
